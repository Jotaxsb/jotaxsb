...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:Jota\n' // Nama kamu
            + 'ORG:Lexa;\n' // Nama bot
            + 'TEL;type=CELL;type=VOICE;waid=5521920025171:+55 21 92002-5171\n' //Nomor whatsapp kamu
            + 'END:VCARD'
prefix = '/','*','.'
blocked = []
limitawal = '999999999'
...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
					wait: '❬⏳❭Jota está fazendo aguarde...',
					success: '️❬ ✔ ❭ Feito 🖤',
					...
					levelnol: '*error* 0 °-°',
					error: {
				stick: '*tente novamente ^_^*',
				Iv: '𝗠𝗮𝗮𝗳 𝗹𝗶𝗻𝗸 𝘁𝗶𝗱𝗮𝗸 𝘃𝗮𝗹𝗶𝗱☹️'
				},
				only: {
					group: '[❗] Somente em grupos desculpe! ❌',
					...
					userB: `──「 LISTA 」──\nOlá Jota !\nDesculpe, irmã. Você não está registrado como amigo de Jota. Registre-se para fazer amizade com o bot Jota por meio, \n\nCommand : ${prefix}daftar nama|idade\nExemplo : ${prefix}daftar Jota|14\n\n──「 JOTA BOT 」──`,
					admin: '[❗] Somente adms amigo! ❌',
					Badmin: ' [❗] Desculpe tenho q me torna adm antes! ❌',
				}
  ...
  conn.sendMessage(id, hasil, MessageType.text); 
	...
	if (messagesC.includes("bosta")){
		...
		if (messagesC.includes("bot")){
			...
			client.updatePresence(from, Presence.composing)
			data = fs.readFileSync('./src/Jotajokes.js');
...
break
				...
				case 'wibu':
                   ...
                   break
					case 'totaluser':
					if (!isOwner) return reply(mess.only.ownerB)    
					teks = `\`\`\`╭────*「 *TOTAL DE USUÁRIOO Jota BOT 👑* 」\n\`\`\``
					no = 0
...
// recursos anime, random @Jota domina vida
          ...
          reply('Espere..')
			    ...
			    case 'dono':
                    ...
                    client.sendMessage(from, 'Meu dono é esse gasoso aí tal de "Jota"🙄, chama ele aí ele diz que é "Shitpost"',MessageType.text, { quoted: mek} )
                    break
                case 'fitnah':	
				case 'fake':          
                    if (!isGroup) return reply(mess.only.group)
				...
				case 'neko':
					...
					client.sendMessage(from, tod, image, { quoted: mek, caption: '*╭────*「 *ADMINBOT Jota ✨* 」\n*│+ wa.me/5521920025171*╰──────*「 *Jota* 」*────*\n\n*_SE QUER SER ADMIN DO BOT Jota_*\n*_Tipo /iklan_*' })
					break
				case 'iklan':
				...
				case 'canal':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '️*canal do Jota:*\n\n https://youtube.com/channel/UCZEtf9AlsC2zsJQwrfW-44w'})
					break
				case 'nsfwloli1':
					memein = await kagApi.memeindo()
				...
				case 'dono':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*WPP:* wa.me/+5521920025171\n*\n\nEspero que tenham gostado do bot️'})
					break
				case 'setprefix':
					if (args.length < 1) return
					if (!isOwner) return reply(mess.only.ownerB)
